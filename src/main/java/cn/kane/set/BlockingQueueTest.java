...
public class BlockingQueueTest {  
    ...
    public static void testBasket() {  
        // 建立一个装苹果的篮子  
        final  BlockingQueue<String> basket = new  ArrayBlockingQueue<String>(3);  
...
class Producer implements Runnable {  
	...
	private BlockingQueue<String> basket = null ;
	public Producer(BlockingQueue<String> basket ){
		this.basket = basket ;
	}
    public void run() {  
        try {  
            ...
            while (true) {  
                ...
                basket.put("produce apple-"+appleNo);  
...
class Consumer implements Runnable {  
	private BlockingQueue<String> basket = null ;
	public Consumer(BlockingQueue<String> basket ){
		this.basket = basket ;
	}
    public void run() {  
        try {  
            while (true) {  
                ...
                String message =basket.poll();  
                System.out.println("消费者消费苹果完毕：" + message + System.currentTimeMillis());  
                // 休眠1000ms  
                Thread.sleep(1000);  
            }  
        } catch (InterruptedException ex) {  
